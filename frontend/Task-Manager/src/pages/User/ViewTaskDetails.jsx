...
const ViewTaskDetails = () => {
  ...
  const getStatusTagColor = (status) => {
    switch (status) {
      ...
      case "Completed":
        return "text-lime-500 bg-lime-50 border border-cyan-500/20"

      default:
        return "text-violet-500 bg-violet-50 border border-cyan-500/10"
    }
  }

  const getTaskDetailsById = async () => {
    try {
      const response = await axiosInstence.get(API_PATHS.TASKS.GET_TASK_BY_ID(id))
  ...
  const updateTodoChecklist = async (index) => {
    ...
    try {
      const response = await axiosInstence.put(API_PATHS.TASKS.UPDATE_TODO_CHECKLIST(taskId),
        { todoChecklist }
      )
